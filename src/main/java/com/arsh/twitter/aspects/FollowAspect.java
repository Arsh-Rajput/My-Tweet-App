...
public class FollowAspect {
	...
	@Around(value = "execution(* com.arsh.twitter.services.FollowService.*(..))")
	...
	@AfterThrowing(value = "execution(* com.arsh.twitter.services.FollowService.*(..))", throwing = "exception")
	public void logException(JoinPoint joinPoint, Exception exception) throws IOException {
		Object[] obj = joinPoint.getArgs();

		if (obj.length > 0) {
			if (obj[0] instanceof Tweet) {
				FileUtility.writeToFile(FileUtility.getFollowExceptionFile(),
